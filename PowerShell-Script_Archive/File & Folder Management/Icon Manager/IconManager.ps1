...
$btnBuild.Add_Click({
    try {
        ...
        if (-not $pythonPath) {
            throw "Build failed: python.exe not found in your system's PATH. The python-based build engine is required."
        }
        ...
        $pyScriptPath = [System.IO.Path]::Combine($ScriptBaseDir, "ico2dll.py")

        if (-not (Test-Path $pyScriptPath)) { throw "Build failed: ico2dll.py not found." }
        
        # --- 1. PREPARE ICONS DIRECTORY ---
        Update-Status "Step 1/2: Preparing icons directory..."
        if (-not (Test-Path $iconsDir)) { New-Item -Path $iconsDir -ItemType Directory -Force | Out-Null }
        ...
        foreach($iconPath in $iconsToBuild) {
            ...
            $destinationPath = [System.IO.Path]::Combine($iconsDir, $fileName)
            Copy-Item -Path $iconPath -Destination $destinationPath -Force
            # Add the path in quotes for the command line argument
            $iconFilePathsForPy += "`"$destinationPath`""
        }

        # --- 2. BUILD DLL USING ICO2DLL.PY ---
        Update-Status "Step 2/2: Building DLL with ico2dll.py engine..."
        # We need to call python and pass the script path as the first argument
        $pythonArgs = "`"$pyScriptPath`" -o `"$dllOutputPath`" $($iconFilePathsForPy -join ' ')"
        ...
        if ($process.ExitCode -ne 0) {
            ...
            throw "Build failed: ico2dll.py exited with code $($process.ExitCode). Error: $errorOutput"
        }
