...
# -----------------------------------------------------------------------------
$ScriptBaseDir = [System.AppDomain]::CurrentDomain.BaseDirectory.TrimEnd('\')
...
$IconsDir = [System.IO.Path]::Combine($ScriptBaseDir, "Resources")
...
Archive-OldLogs
Write-Log "Application Started. EliteSoftware MP3 Toolkit v1.9.1.0"
...
function Load-Icon {
    ...
    if (Test-Path $Path) {
        try {
            if ($Size -gt 0) {
                # Force specific size (e.g., 256x256 for banners)
...
function Create-GradientHeader {
    param($ParentForm, $TitleText, $IconName)
    ...
    # Icon (PictureBox) - Load 256x256 specifically for banner
    if ($IconName) {
...
# We assign to $null or cast to [void] to prevent the EXE from popping up "Output" dialogs
$FileMenu = $MenuStrip.Items.Add("File")
[void]$FileMenu.DropDownItems.Add("Open Folder...", $null, { Open-Folder })
[void]$FileMenu.DropDownItems.Add("Save Copy to Safe House...", $null, { Save-File })
[void]$FileMenu.DropDownItems.Add("-")
[void]$FileMenu.DropDownItems.Add("Exit", $null, { $Form.Close() })

$HelpMenu = $MenuStrip.Items.Add("Help")
[void]$HelpMenu.DropDownItems.Add("Guidance System", $null, { Show-Help })
[void]$HelpMenu.DropDownItems.Add("What's New?", $null, { Show-WhatsNew })
[void]$HelpMenu.DropDownItems.Add("About", $null, { Show-About })
# -- Header Panel --
...
$SplitContainer.SplitterDistance = 360 # Wider tree view
...
$TreeContainer.Padding = New-Object System.Windows.Forms.Padding(0, 45, 0, 0) # Fix top cutoff
...
$btnOpenFolder.Text = "Import Media Folder"
...
$ArtGroup.Text = "Album Cover Artwork"
...
$MetaGroup.Text = "File Metadata"
# Significantly increased height to fit restored features
$MetaGroup.Size = New-Object System.Drawing.Size(550, 750) 
...
# --- RESTORED AND MERGED FIELD LIST ---
...
Create-MetaField "Disc #:" 350 "Disc"
Create-MetaField "BPM:" 390 "BPM"
Create-MetaField "Subtitle:" 430 "Subtitle"
Create-MetaField "Publisher:" 470 "Publisher"
Create-MetaField "Composer(s):" 510 "Composer"
Create-MetaField "Conductor(s):" 550 "Conductor"
Create-MetaField "Copyright:" 590 "Copyright"
Create-MetaField "Comment:" 630 "Comment"

# Make Comment box taller and multiline
$global:MetaControls["Comment"].Multiline = $true
$global:MetaControls["Comment"].Height = 50
$global:MetaControls["Comment"].ScrollBars = "Vertical"

# Star Rating (ComboBox)
$lblRating = New-Object System.Windows.Forms.Label
$lblRating.Text = "Star Rating:"
$lblRating.Location = New-Object System.Drawing.Point(20, 690)
$lblRating.AutoSize = $true
$MetaGroup.Controls.Add($lblRating)

$cmbRating = New-Object System.Windows.Forms.ComboBox
$cmbRating.Location = New-Object System.Drawing.Point(140, 687)
$cmbRating.Size = New-Object System.Drawing.Size(300, 25)
$cmbRating.DropDownStyle = "DropDownList"
# Simple 0-5 Star Mapping
$cmbRating.Items.AddRange(@("Unrated", "★", "★★", "★★★", "★★★★", "★★★★★"))
$MetaGroup.Controls.Add($cmbRating)
$global:MetaControls["Rating"] = $cmbRating
$cmbRating.Add_SelectedIndexChanged({ Save-To-Memory })

# -- Playback Controls --
$PlayerGroup = New-Object System.Windows.Forms.GroupBox
$PlayerGroup.Text = "Media Playback"
$PlayerGroup.Size = New-Object System.Drawing.Size(310, 160)
...
if ($Icon_Spin) {
    ...
    $picSpin.Location = New-Object System.Drawing.Point(245, 30)
...
$btnSave.Location = New-Object System.Drawing.Point(600, 600)
...
$btnSave.TextAlign = "MiddleRight"
$btnSave.UseVisualStyleBackColor = $true
if ($Icon_Save) {
    $btnSave.ImageAlign = "MiddleCenter"
...
$global:IsLoading = $false # Flag to prevent save during load

function Save-To-Memory {
    if (-not $global:SelectedFile) { return }
    if ($global:IsLoading) { return } # Don't save during load
...
function Open-Folder {
    # Use the Shell COM Object to force the modern "Vista Style" picker
    $Shell = New-Object -ComObject Shell.Application
    # 0 = Desktop Root
    # "Select your MP3 Stash" = Title
    # 0x00000040 = BIF_NEWDIALOGSTYLE (This forces the modern UI with resizing/sidebar)
    $Folder = $Shell.BrowseForFolder(0, "Select your MP3 Stash", 0x00000040, 0)
    
    if ($Folder) {
        $SelectedPath = $Folder.Self.Path
        
        $TreeView.Nodes.Clear()
        $RootNode = $TreeView.Nodes.Add($SelectedPath)
        $RootNode.ImageKey = "folder"
        $RootNode.SelectedImageKey = "folder"
        
        $Files = Get-ChildItem -Path $SelectedPath -Filter "*.mp3"
        ...
        $RootNode.Expand()
        $StatusLabel.Text = "Opened folder: $SelectedPath"
        Write-Log "Opened folder: $SelectedPath"
    }
}
...
function Load-Metadata {
    ...
    foreach ($key in $global:MetaControls.Keys) {
        if ($global:MetaControls[$key] -is [System.Windows.Forms.TextBox]) {
            $global:MetaControls[$key].Text = ""
        }
    }
    $global:MetaControls["Rating"].SelectedIndex = -1
    ...
    if ($global:MetaMap.ContainsKey($global:SelectedFile)) {
        ...
        $global:MetaControls["Disc"].Text = $State["Disc"]
        ...
        $global:MetaControls["Composer"].Text = $State["Composer"]
        $global:MetaControls["Conductor"].Text = $State["Conductor"]
        $global:MetaControls["Copyright"].Text = $State["Copyright"]
        $global:MetaControls["Comment"].Text = $State["Comment"]
        $global:MetaControls["Rating"].SelectedIndex = $State["Rating"]
    ...
    try {
        ...
        $global:MetaControls["Disc"].Text = $tag.Disc
        ...
        # --- FIX: USE POWERSHELL -JOIN TO HANDLE NULL ARRAYS SAFELY ---
        $global:MetaControls["Composer"].Text = ($tag.Composers -join ", ")
        $global:MetaControls["Conductor"].Text = ($tag.Conductors -join ", ")
        # --------------------------------------------------------------
        
        $global:MetaControls["Copyright"].Text = $tag.Copyright
        $global:MetaControls["Comment"].Text = $tag.Comment
        
        # Default rating to 0 (Unrated) for now
        $global:MetaControls["Rating"].SelectedIndex = 0
...
function Save-File {
    ...
    if ($FolderBrowser.ShowDialog() -eq "OK") {
        ...
        try {
            ...
            $tag.AlbumArtists = @($global:MetaControls["AlbumArtist"].Text)
            ...
            $tag.Disc = [uint32]$global:MetaControls["Disc"].Text
            $tag.BeatsPerMinute = [uint32]$global:MetaControls["BPM"].Text
            $tag.Subtitle = $global:MetaControls["Subtitle"].Text
            $tag.Publisher = $global:MetaControls["Publisher"].Text
            $tag.Composers = @($global:MetaControls["Composer"].Text)
            $tag.Conductor = @($global:MetaControls["Conductor"].Text)
            $tag.Copyright = $global:MetaControls["Copyright"].Text
...
function Show-About {
    ...
    $ContentBox.Text = "EliteSoftware - MP3 Toolkit`nVersion: v1.9.1.0`n(c) 2025 EliteSoftware - Zachary Whiteman - Gemini AI.`n`nAn application for discovering and extracting joy from MP3 files.`n`nWarning: This is ALPHA software. Use at your own risk."
    ...
    $btnDebug.Add_Click({ [System.Windows.Forms.Clipboard]::SetText("Debug Info: EliteSoftware MP3 Toolkit v1.9.1.0") })
...
function Show-WhatsNew {
    ...
    Create-GradientHeader $NewsForm "Feature Changelog" "WHATS NEW.ico"
    ...
    $ContentBox.BackColor = $Color_Control
    $ContentBox.Text = "--- What's New in v1.9.1.0 ---`n- CRITICAL FIX: Album Art not loading due to Composer/Conductor crash.`n- Switched to safer text joining method.`n`n--- v1.9.0.0 ---`n- Restored missing features: BPM, Publisher, Subtitle, Copyright, Conductors, and more!`n- Added 'Disc Number' field.`n- Merged strict saving logic with expanded metadata support.`n`n--- v1.8.0.0 ---`n- STRICT Metadata Logic.`n- High-Res Icons.`n`n--- v1.0.0.0 ---`n- Initial Release."
...
function Show-Help {
    ...
    Add-Tab "Version History" "v1.9.1.0 (2025-11-20)`r`n- Album Art & Metadata Crash Fix.`r`n`r`nv1.9.0.0`r`n- Restored missing fields (BPM, Copyright, etc.).`r`n`r`nv1.8.0.0`r`n- Strict Metadata & High-Res Icons.`r`n`r`nv1.7.0.0`r`n- High-Res Icons & Tree Fixes.`r`n`r`nv1.0.0.0`r`n- Initial Release."
